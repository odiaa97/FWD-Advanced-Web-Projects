  ...
  constructor() {
    ...
    this.deleteProduct = (req, res) =>
      __awaiter(this, void 0, void 0, function* () {
        try {
          let productId = parseInt(req.params.id);
          const productExists = yield this.productStore.checkIfProductExists(
            productId
          );
          if (productExists) {
            (0, getCurrentUser_1.getUserRole)(req)
              ...
              .catch((e) => {
                return res
                  .status(401)
                  .json(`Cannot delete product: ${e.message}`);
              });
          } else {
            return res.status(404).json('Product not found');
          }
      ...
      __awaiter(this, void 0, void 0, function* () {
        try {
          ...
          if (productToBeUpdated.id) {
            const product = yield this.productStore.getProduct(
              productToBeUpdated.id
            );
            if (product) {
              const result = yield this.productStore.updateProduct(
                productToBeUpdated
              );
