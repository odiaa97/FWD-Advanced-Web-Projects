...
class ProductController {
  ...
  getProduct = async (req: Request, res: Response) => {
    try {
      ...
      const product: Product = await this.productStore.getProduct(productId);
  ...
  createProduct = async (req: Request, res: Response) => {
    try {
      const newProduct: Product = { name: req.body.name, price: req.body.price };
      const productAdded = await this.productStore.createProduct(newProduct);
  ...
  deleteProduct = async (req: Request, res: Response) => {
    try {
      const productId: number = parseInt(req.params.id);
      const productExists = await this.productStore.checkIfProductExists(
        productId
      );
      if (productExists) {
        getUserRole(req)
  ...
  updateProduct = async (req: Request, res: Response) => {
    try {
      const productToBeUpdated: Product = req.body;
      if (productToBeUpdated.id) {
        const product = await this.productStore.getProduct(
