...
class CartController {
            ...
            getCurrentUser(req, res)
                .then(async (user) => {
                    ...
                    if (user.id && user.id == order.user_id) {
                        console.log(user.id);
                        const products = await this.cartStore.getCartProductsInOrder(
                            user.id
                        );
    ...
    changeQty = async (req: Request, res: Response) => {
        try {
            ...
            const orderUpdated = await this.cartStore.changeProductQty(
                orderProduct,
                sign
            );
    ...
    getOrderProduct = async (req: Request, res: Response) => {
        try {
            getCurrentUser(req, res)
                .then(async (user) => {
                    const orderProduct = await this.cartStore.getOrderProduct(
                        req.body.id
                    );
    ...
    addProductInCart = async (req: Request, res: Response) => {
        try {
            const orderProduct = req.body;
            const newOrderProduct = await this.cartStore.addProductInCart(
                orderProduct
            );
    ...
    deleteOrderProduct = async (req: Request, res: Response) => {
        try {
            ...
            const orderProductDeleted = await this.cartStore.deleteOrderProduct(
                orderProduct
            );
    ...
    deleteProductInAllCarts = async (req: Request, res: Response) => {
        ...
        const allRowsDeleted = await this.cartStore.deleteProductInAllCarts(
            productId
        );
