...
class UserController {
    ...
    index = async (_req: Request, _res: Response) => {
        getCurrentUser(_req, _res).then(user => {
            if (user) {
                getUserRole(_req).then(async role => {
    ...
    updateUser = async (req: Request, res: Response) => {
        try {
            const user: User = { id: req.body.id, username: req.body.username, password: req.body.password, email: req.body.email };
