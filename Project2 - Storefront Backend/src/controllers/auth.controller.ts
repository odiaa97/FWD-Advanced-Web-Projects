...
class AuthController {
    ...
    register = async (req: Request, res: Response) => {
        const user: User = { username: req.body.username, password: req.body.password, email: req.body.email, role: req.body.role };
        if (!await auth.checkIfUserExists(user.username)) return res.status(400).json("User already exists");
        auth.register(user)
            .then((result) => {
