...
const orderProduct: Order_Product = { id: 1, order_id: 2, product_id: 2, quantity: 1 };
...
describe('ORDER_PRODUCT MODEL TEST', () => {
    ...
    it('Should add product in order', async () => {
        expect(await cartStore.addProductInCart(orderProduct)).toEqual(orderProduct);
    });

    it('Should increment quantity', async () => {
        const updatedOrderProduct = await cartStore.changeProductQty(orderProduct, true);
        orderProduct.quantity++;
        expect(updatedOrderProduct.quantity).toEqual(orderProduct.quantity);
    });

    it('Should decrement quantity', async () => {
        const updatedOrderProduct = await cartStore.changeProductQty(orderProduct, false);
        orderProduct.quantity--;
        expect(updatedOrderProduct.quantity).toEqual(orderProduct.quantity);
    });
});